...
func makeTopFrame(setup ClockSetup) sdf.SDF3 {
	...
	shaftHolder := sdf.Cylinder3D(setup.Frame.ShaftHolderLength-setup.Frame.Thickness/2, setup.Shaft.OD*1.5, setup.Shaft.OD/2)
	shaftHole := sdf.Cylinder3D(setup.Frame.ShaftHolderLength, setup.Shaft.OD/2, 0)
